# Aloe specification: unified nominal families

> **NORMATIVE TARGET — 89I RATIFICATION CHANGE SUBMITTED FOR REVIEW**
>
> This specification integrates the complete audited family design, including
> approved U1, with the preserved language contracts. Acceptance of this change
> completes checkpoint 89's design/documentation arc. Executable behavior
> remains complete through checkpoint 88: `define-family`, family `case`, send
> type headers, and `invoke-mirrored` are not implemented by this change.
> [CHECKPOINTS.md](CHECKPOINTS.md) governs implementation order. 90A is the next
> planned slice and needs its own checkpoint document and authorization.

Aloe is an s-expression language whose evaluation is receiver-first message
send. The prototype remains a Racket definitional interpreter and type checker;
there is no new release designation, compiler, mutation, inheritance, or macro
system in this promotion.

This is the single language-law document. The [historical candidate and
promotion map](docs/unified-nominal-adts-spec-candidate.md#promotion-map) preserve
the accepted pre-ratification text; the [implementation handoff](docs/unified-nominal-adts-implementation-handoff.md)
provides repository navigation and transition guidance. The unchanged audit,
U1 record, and archive are design provenance, not alternative specifications.

Historical bare section references refer to `SPEC.md` at revision
`616c0ceecd7d52d7f9c3e2c4df89e9037ed57aaa`. Section numbers below follow the
candidate's 1–22 organization. Compatibility anchors locate preserved rules;
historical terminology does not add legacy syntax to the target grammar.
Known runtime gaps, including written function annotations, retained value
types, and strict closure, remain assigned to their reviewed roadmap owners.

## 1. Preserved expression model

The language preserves Aloe's existing atoms and lexical lookup. Integers,
...
`f`. The language adds no constructor-call form, computed selector, implicit
...
declaration; this specification changes how a family declares conformance, not how
...
The language introduces only two expression-level syntactic distinctions:
...
### Preserved syntax and forms

A program is a sequence of s-expressions read by the Scheme/Racket reader.
Integer examples are `0`, `10`, and `-3`; Float examples are `0.0`, `1.0`, and
`0.01`; strings include `"x"` and `"hello"`; booleans are `#t` and `#f`.
Source symbols such as `demo`, `x`, `+`, and `Point` retain lexical lookup
and literal-selector roles. Capitalization of type and family names is a
convention, not an enforced restriction.

The following forms retain their existing syntax. Family/protocol declarations
and family/List extensions are specified in sections 3, 7, and 15; case is
specified in section 4. A method, factory, function, `let`, or case-clause body
is one expression. Nested `let` introduces locals; there is no `begin`.
...
Bind the name in the top-level environment to the evaluated expression; its
type is inferred from that expression. There is no `(define (f x) ...)` sugar.
Evaluation begins only after the transaction has passed section 9's checks.
...
A function object executes only through `(f call arg ...)`. Parameter and
return types are optional; omitted types are inferred from an expected type
or the body. If neither determines a unique type, checking fails. Arrow-typed
parameters supply expected types to function arguments, and written
annotations are checked. Section 8's closure boundaries apply. `(f x)` still
sends the literal selector `x` and does not call the function.
...
Bindings are parallel: each right-hand side sees the outer environment, not
other names introduced by the same `let`. Each binding's type is inferred
from its expression, and the result is the body's value. Its runtime definition
is the `fn`/`call` expansion above; source alias provenance must survive checking
...
This is sugar for `(test if (fn () then) (fn () else))`. The Boolean receiver
selects exactly one zero-argument function to receive `call`. Both branches
are checked with a common result type. `cond` is sugar for nested `if` forms
and requires its final `else` clause. Enclosing type constraints and shallow
branch joins follow sections 4 and 5 without changing lazy evaluation.
...
Read, check, and evaluate the loaded file in the same environment. Resolve
relative paths from the loading file before the current directory and project
root. A complete unit includes its transitive loads and is checked and
finalized before any evaluation, as specified in section 9.
...
Require same-typed operands, then evaluate them left to right and compare with
section 12's kernel equality. Success returns the right-hand value. Failure
reports both original source datums and both resulting values. A user-defined
`=` row does not replace this comparison. Static failure prevents evaluation.
...
The language distinguishes three declaration-owned callable surfaces:
...
The language's only representation-elimination form is:
...
The language generalizes the existing source type grammar without admitting
...
`define-protocol` is a top-level, non-generic declaration. It declares a name
and zero or more required typed signatures, using section 3's method-row shape
without a body. Empty marker protocols remain legal:

```aloe
(define-protocol Math)
```

A separate example with requirements is:
...
A zero-argument signature uses the explicit `()` marker. A requirement may
use the existing row-local type header; this does not make the protocol itself
generic. Families opt in through `conforms`, under the uniform full-domain
rules below. Protocols describe types, not a second runtime method table:
checking a protocol-view receiver uses its requirements, while concrete
runtime dispatch remains on the receiver's family. There are no default
implementations, inherited fields, or `super`.
...
is rejected. No match is a type error during checking; an ordinary runtime
lookup with no matching message reports an unknown-message error.
Compatibility requires exact types or the declared nominal protocol relation;
there is no implicit numeric conversion. This is
...
In the established MPL vocabulary, `Sum` has `+` overloads for `Int` and `Sym`.
Under its complete definitions with `x : Sym`, `((x + 2) + 3)` selects the
`Int` overload and `((x + 2) + x)` selects `Sym`. Equivalent migrated family
definitions retain those observations subject to the coherence rules below;
no implicit `Int`-to-`Math` lift is added.
...
The implementation remains a Racket definitional interpreter plus a type
checker. Object sends are not elaborated into Racket evaluation. The old
parse/check/raw-interpreter sketch is replaced by the checked pipeline below.
Source `let` must not lose alias provenance before checking.

The language requires checking to resolve every static decision needed by
...
This specification defines the descriptors' semantic contents and shared opaque
...
The language promises no stable family, constructor, protocol, or row
...
The default REPL printer uses the applicable `show` path when present;
the REPL's `:raw` command uses the structural renderer independently.
...
The language adds exactly these metadata queries:
...
type description. Signatures may be stored and copied, but this specification
...
type-object value in expression position. This specification creates no new such
...
### Numeric and List operation reference

`Int` and `Float` retain the following binary messages:
...
Operands have the same numeric kind. Arithmetic returns that kind; comparisons
return `Bool`. There is no implicit coercion. `(n float)` converts Int to Float.
In the established Point vocabulary, `+` and `-` combine points, `*` and `/`
take a scalar of `T`, and `dist2` returns squared distance as `T`. Boids uses
`(avg-pos / (n float))`, not `(avg-pos / n)`. These application methods do not
introduce a numeric constraint language or make `Int` conform to MPL's `Math`.
The preserved generic Point methods assume `T` understands `+ - * /` as
`Int` and `Float` do; Boids instantiates `T = Float`.

For `xs : (List T)`, the preserved operations are:

| Send | Meaning | Result |
| --- | --- | --- |
| `(List of x ...)` | Homogeneous variadic construction | `(List T)` when the elements share `T` |
| `(List empty)` | Empty construction with an element type determined within its checking unit | `(List T)` |
...
| `(xs cons x)` | New list with a same-typed `x` prepended | `(List T)` |
| `(xs map f)` | Send `call` to `f` for each element | `(List U)` when `f : (-> T U)` |
| `(xs fold acc f)` | Left fold using `(f call acc element)` | The accumulator type |

`fold`, `reverse`, and `map` remain library methods; only `of`, `empty`,
`empty?`, `first`, `rest`, `cons`, and `len` are host-provided List primitives.
The existing `define-methods List` route below keeps `T` in scope and permits
typed method rows with fresh method-local parameters. Lists remain immutable
and invariant, including contextual empties; no unresolved element type may
escape section 8's closure boundaries.

`String` remains a primitive value with its existing public messages.
`Symbol` has no reader literal: `(Symbol intern string)` takes a `String`,
`(sym name)` returns that name string, and `(sym = other)` compares symbols.
Equal strings intern to the same Symbol object. String and Symbol values do
not alter source-symbol lookup or literal selectors.
...
These examples specify target syntax, reflection, and integration contracts.
Family declarations and the new reflection behavior remain unimplemented at
checkpoint 88; these are future acceptance examples, not current runnable
programs. Established built-in and host observations are preservation examples.
...
The following checkpoint-88 and target method fragments isolate the approved
reflection transition; their surrounding nominal declarations and the separate
class-to-family migration are omitted. The replacement fragments are future
acceptance examples. This documentation change does not modify Gel source.

Checkpoint-88 Gel helpers (historical comparison) use ordinary results and
contextual unwrapping:
...
The approved replacements keep intermediate values behind the guarded
...
client path; it does not waive the language's closure rule for ordinary
...
In the unified family model, a `(List (Option Int))` can hold both `None`
...
supplies that type directly. Under the language's closure rule, an empty
...
This specification defines the unified target. The checkpoint-88 runtime
continues to accept its historical syntax until the assigned implementation
transitions; this documentation change does not alter parser acceptance.

The superseded and replacement surfaces are:

| Superseded class surface | Normative family target |
| --- | --- |
...
The completed target language rejects `define-class`, positional
...
The implementation sequence introduces a temporary source bridge in 91A,
lowering legacy `define-class` declarations to the same internal one-constructor
family model with explicit `new`. Repository source migrates in 103A/103B;
103C removes the bridge and all legacy acceptance. This is development
sequencing, not target grammar or a compatibility promise. No second user
nominal runtime is permitted. The governing order is in
[CHECKPOINTS.md](CHECKPOINTS.md#future-family-implementation); each entry needs
its own authorized checkpoint. No bridge is implemented by this ratification.

The language preserves all unaffected expression behavior, including literal
...
adds no language features beyond those rules. A diagnostic
...
This specification prescribes no numeric codes, exact diagnostic wording,
...
The completed-source exclusions do not deny section 17's temporary
development bridge, and this catalogue does not authorize that bridge.
...
The following are absent from the target language, not claims that they
can never be designed later. Required protocol signatures and multiple
family conformances are included in this specification; their historical absence
from earlier Aloe versions is not a current design exclusion. Likewise,
today's unimplemented family features remain part of the specified destination.

| Subject | Excluded target-language facilities | Governing rules |
| --- | --- | --- |
...
These are required future implementation validation scenarios, not current
checkpoint-88 goldens or a test-harness design. They pair the rejection
catalogue with permitted behavior. Unless a row supplies another context, family examples
...
They require future implementation validation; this documentation change
does not execute them against the checkpoint-88 runtime.

| Scenario and context | Required observation | Governing rules |
| --- | --- | --- |
| Check all three target Gel helper bodies in section 16 and their unchanged zero-argument, plain-argument, and mirrored-argument call sites. | No source inference variable escapes. The fixed parameter supplies `(List Mirror)` context to `List empty`; both direct helpers return `Mirror` and select the Mirror `push` overload. The generic helper wraps its bound `T` argument and delegates to the Mirror overload. | Sections 8, 14, and 16 |
...
These implementation checks are distinct from the completed Decision 1–10
design audit, retained as provenance in the
[89F record](docs/unified-nominal-adts-design-audit.md).

## 22. Application validation and eventual completion evidence

Implementation completion requires application evidence as well as isolated
semantic scenarios. These obligations are future work under separately
authorized checkpoints; documentation ratification does not execute them.
...
Acceptance of the 89I ratification change completes the design/documentation
arc; it does not supply this implementation evidence.
...
Every implementation slice must preserve the specified boundaries and reject
unavailable behavior rather than approximate it with name-based identity,
partial case, reflected locals, or unresolved construction. This section
records validation obligations, not detailed checkpoint instructions, future
test filenames, algorithms, or a handoff. [CHECKPOINTS.md](CHECKPOINTS.md)
and the [supporting roadmap](docs/unified-nominal-adts-implementation-roadmap.md)
own the implementation order and evidence allocation.

### Preserved Point and Boids observations

The repository's `examples/point.aloe` and `examples/boids.aloe` are runnable
checkpoint-88 programs until their assigned migration. The target retains
these observations under equivalent explicit family declarations preserving
their fields and methods. Section 16's Point is a smaller illustrative family;
the Boids observations below assume the complete Boids definitions or equivalent
stubs, including `demo : Sim`, generic Point/Boid, and a `(List (Boid Float))`
flock. They do not add implicit constructors to the target.

Required positive observations:

1. `(Point new 1 2)` produces a `(Point Int)`.
2. `((Point new 1 2) x)` returns `1`.
3. `((Point new 1.0 2.0) + (Point new 3.0 4.0))` produces `(Point new 4.0 6.0)`.
4. `((List of 1 2 3) len)` returns `3`.
5. Both trailing `(demo step)` sends in Boids produce the expected `Sim` results.

Required type errors under those definitions:

1. `(Point new 1 2.0)` has inconsistent `T`.
2. `(demo len)` sends a selector absent from `Sim`.
3. `((Point new 1 2) position)` sends a selector absent from `Point`.
4. `(List of 1 2.0)` mixes element types.
5. `((Point new 1 2) + (Point new 3.0 4.0))` mixes invariant `(Point Int)` and
   `(Point Float)` arguments.
