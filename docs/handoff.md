...
- Tests through checkpoint 103 are green on `experiment/filesystem`. `Option`
  is loadable from `lib/option.aloe` and is not a default-driver binding.
  `lib/fs.aloe` defines the `Path` product and generic `Fs` wrapper with
  `current`, `path`, `child`, `parent`, and `name`; the optional production
  `fs-host` receiver and its test double remain explicitly injected. Default
  drivers still receive neither filesystem nor terminal authority. Tag
  `v0.1.0-boids` records the earlier 0.1 milestone.
...
loadable Aloe library, and `Path` plus the generic `Fs` wrapper now provide
the public `current`, `path`, `child`, `parent`, and `name` path messages over
an explicitly injected production `fs-host` or controlled test double.
`Entry`, `inspect`, and `entries` do not exist yet. The host crossing vocabulary
remains `Int`, `Bool`, `String`, and homogeneous `(List String)`. Option remains
loadable rather than bootstrapped. Constructors are not yet ready to merge to
`main`.
